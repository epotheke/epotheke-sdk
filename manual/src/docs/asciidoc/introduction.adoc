...
==== Tenant token
The tenant token is used for access control at the epotheke service and can be obtained from the service provider.
...
|requestCardInsertion(session: SmartCardSalSession): SmartCardDeviceConnection
|A connection to the eGK has to be provided.
|A connection can be established by the use of provided `SmartCardSalSession` object.
There is a default implementation `SmartcardSalHelper.connectFirstTerminalOnInsertCard` which can be used if there is only one terminal and a card should be connected, as soon as it is detected.

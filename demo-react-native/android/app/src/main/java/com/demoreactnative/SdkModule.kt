...
class SdkModule(
    ...
    private suspend fun callPrescriptionProtocolNullChecked(
        p: Promise,
        call: suspend PrescriptionProtocol.() -> Unit,
    ) {
    ...
    private val cardLinkControllerCallback =
        object : CardLinkControllerCallback {
...
//            onAuthenticationCompletionCB?.invoke(minor, msg)
    ...
    private val cardLinkInteraction =
    ...
    private val errorHandler =
        object : SdkErrorHandler {
            override fun onError(error: ServiceErrorResponse) {
                logger.debug {
                    "rn-bridge: SdkModule onError will call registered RN callback with: ${error.errorMessage}"
                }
